...
func handleUpdateUser(c *Context) {
	...
	usernameAdditional, usernameErr := c.Auth.usernameAdditionalFromRaw(c.R.Context(), body, user.ID, false)
	if usernameErr != nil {
		c.WriteError(usernameErr)
		return
	}
	update.Additional = mergeAdditionalMaps(update.Additional, usernameAdditional)
	phoneAdditional, phoneErr := c.Auth.phoneNumberAdditionalFromRaw(body)
	if phoneErr != nil {
		c.WriteError(phoneErr)
		return
	}
	update.Additional = mergeAdditionalMaps(update.Additional, phoneAdditional)
