...
Custom values are stored in the portable JSON `additional` column. Built-in
plugin fields such as `role`, `username`, or `twoFactorEnabled` use the
dedicated columns generated by the matching plugin schema. Set `Input: false`
on a field to prevent client writes.
